...
def save_faiss(docs, embedder):
    ...
    # Lấy metadata và THÊM NỘI DUNG CHUNK VÀO METADATA
    metas = [d.metadata.copy() for d in docs] 
    for i, meta in enumerate(metas):
        meta["content"] = texts[i] # <-- Dòng quan trọng: Lưu nội dung chunk
...
class Verifier:
    ...
    def entailment(self, premise, hypothesis):
        out = self.pipe(hypothesis + " </s> " + premise)[0]
        for o in out:
...
def chat():
    ...
    while True:
        ...
        # Dense retrieval
        D, I_dense = idx.search(q_emb, CONFIG["fetch_k"])
        ...
        for j in I:
            ...
            # --- THAY THẾ TOÀN BỘ LOGIC TẢI LẠI PDF BẰNG DÒNG NÀY ---
